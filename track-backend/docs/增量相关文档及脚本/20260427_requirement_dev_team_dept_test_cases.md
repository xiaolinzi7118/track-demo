...
2. 在参数维护中准备部门数据（`paramId=SYS_DEPT`）：
   ...
   - 至少 1 条 `status=0` 且 `extraAttr!=开发`
3. 准备角色与账号：
   - 开发人员A：`developer` 角色，所属部门=开发部门X
   - 开发人员B：`developer` 角色，所属部门=开发部门Y（与X不同）
   - 业务人员C：`business` 角色，所属部门任意
...
### TC-USER-001 开发角色所属部门限制-通过
- 步骤：给用户分配 `developer` 角色，所属部门选择 `extraAttr=开发` 的部门
- 预期：保存成功

### TC-USER-002 开发角色所属部门限制-拒绝
- 步骤：给用户分配 `developer` 角色，所属部门选择 `extraAttr!=开发` 的部门
- 预期：保存失败，提示所属部门必须为开发属性部门

### TC-USER-003 非开发角色所属部门限制
- 步骤：仅分配非 `developer` 角色，所属部门选择任意部门
- 预期：保存成功
